...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	LAYOUT(
		...
		KC_TRNS, KC_TRNS, KC_TRNS, KC_UP,    RGB_HUD, RGB_SAI, RGB_SAD, RGB_VAI, RGB_VAD, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, RESET,
		KC_TRNS, KC_TRNS, KC_LEFT, KC_DOWN,  KC_RIGHT, RGB_TOG, RGB_MOD, RGB_HUI, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,
	...
	LAYOUT(
	    ALT_T(KC_APP), M(1), M(2), M(3), M(4), M(5), M(6), M(7), M(8), M(9), M(10), M(11), M(12), KC_TRNS, KC_TRNS,
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt) {
	if (record->event.pressed) {
		switch(id) {
		case 1:
			return MACRO(D(LCTL), D(LGUI), T(F1), U(LGUI), U(LCTL), END);
		case 2:
			return MACRO(D(LCTL), D(LGUI), T(F2), U(LGUI), U(LCTL), END);
		}
	}
	return MACRO_NONE;
};
